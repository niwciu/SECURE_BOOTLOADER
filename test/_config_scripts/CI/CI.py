...
# =========================
# WALIDACJA KONFIGURACJI YAML
# =========================
def validate_config(config):
    if not isinstance(config, dict):
        raise ValueError("Plik YAML musi zawierać mapę (dict) na najwyższym poziomie.")

    if "build" not in config:
        raise ValueError("Brak wymaganej sekcji 'build' w pliku YAML.")

    build = config["build"]
    if not isinstance(build, dict):
        raise ValueError("Sekcja 'build' musi być mapą (dict).")

    if "system" not in build:
        raise ValueError("Brak wymaganego parametru 'build.system' (make | ninja).")

    if build["system"] not in ("make", "ninja"):
        raise ValueError("build.system musi mieć wartość: 'make' albo 'ninja'.")

    if "jobs" in build and not isinstance(build["jobs"], int):
        raise ValueError("build.jobs musi być liczbą całkowitą.")

    if "modules" not in config or not isinstance(config["modules"], list):
        raise ValueError("Brak sekcji 'modules' lub nie jest listą.")

    for module in config["modules"]:
        if "name" not in module:
            raise ValueError("Każdy moduł musi posiadać pole 'name'.")
        if "targets" not in module or not isinstance(module["targets"], list):
            raise ValueError(
                f"Moduł {module.get('name', '<unknown>')} musi mieć listę 'targets'."
            )
...
def create_required_directories(config):
    ...
    print(
        f"Struktura folderów została wygenerowana w '{os.path.abspath(base_report_path)}'."
    )


# Funkcja do uruchamiania targetów w folderze "out"
def run_make_targets(module_path, targets, build_system, build_jobs, reconfigure=False):
    out_path = os.path.join(module_path, "out")

    # Usuwamy folder 'out' jeśli istnieje i włączono flagę --reconfigure
    if reconfigure and os.path.isdir(out_path):
        print(f"[--reconfigure] Usuwamy istniejący folder 'out' w {module_path}")
        shutil.rmtree(out_path)

    # Jeśli folder 'out' nie istnieje, tworzymy go przez CMake
    if not os.path.isdir(out_path):
        ...
        if build_system == "ninja":
            ...
            raise ValueError(f"Nieznany system budowania: {build_system}")
    ...
    for target in targets:
        ...
        print(f"Uruchamianie '{' '.join(cmd)}' w {out_path}")
...
"""
        ...
        print(
            f"Informational page about the missing report has been created at {missing_report_path}"
        )
    else:
        print(
            f"Informational page about the missing report already exists at {missing_report_path}"
        )
...
"""
        ...
        for module in modules:
            module_name = module["name"]
            report_file = (
                f"{module_name}.html"  # Nazwa pliku HTML na podstawie nazwy modułu
            )
            ...
            matching_files = [
                f for f in files_in_directory if f.lower() == report_file.lower()
            ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Skrypt do wczytywania konfiguracji z pliku YAML."
    )
    ...
    parser.add_argument(
        ...
        "--reconfigure",
        action="store_true",
        help="Usuń istniejący katalog 'out' i ponownie uruchom CMake",
    )
    ...
    if build_system == "make" and build_jobs is None:
        ...
        print(f"Automatycznie ustawiono make -j{build_jobs}")
